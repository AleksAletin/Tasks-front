  ...
  const onBarDown = (id: string, e: React.MouseEvent) => {
    ...
    const up = (ev: MouseEvent) => {
      ...
      if (dd !== 0 && t && t.phases && t.anchor) {
        updateTask(id, { anchor: { ...t.anchor, date: shiftIso(t.anchor.date, dd) } });
      } else if (dd !== 0 && t && t.tl) {
        updateTask(id, { tl: { start: shiftIso(t.tl.start, dd), end: shiftIso(t.tl.end, dd) } });
      } else if (!moved) {
        openPanel(id);
      }
      setTlDrag(null);
      ...
      <div style={{ overflowX: 'auto', padding: '0 22px' }}>
        <div style={{ minWidth: tl.totalMinW }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div
              style={{ position: 'absolute', top: 0, bottom: 0, left: tl.todayLeftPx, width: 2, background: '#4263d8', opacity: 0.45, pointerEvents: 'none' }}
            />
            ...
            <div style={{ marginTop: 14, paddingTop: 12, borderTop: '2px solid rgba(0,0,0,0.08)', position: 'relative' }}>
              ...
              {tl.resources.map((rs) => (
                <div key={rs.id} style={{ display: 'flex', alignItems: 'center', height: 28 }}>
                  <div style={{ width: LANE_LABEL_W, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8, paddingLeft: 18 }}>
                    <div
                      className="noinv"
                      style={{
                        width: 22,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    {rs.cells.map((cl, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={cl.over ? 'noinv' : undefined}
                          style={{
                            width: DAY_W,
                            height: 16,
                            flexShrink: 0,
                            ...
                            fontSize: 9,
                            fontWeight: 800,
                            boxShadow: today ? 'inset 0 0 0 9999px rgba(66,99,216,0.05)' : undefined,
                          }}
                        >
                          {cl.count}
                        </div>
