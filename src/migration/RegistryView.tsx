// Реестры (brief §4.2): the module registry (id/название/тип/ярус/нужность/роли/задача/состояние,
// filterable) and the role registry (#модулей, ярусная разбивка, %готово). Every row links into
// «Дёрни за ниточку» for the full chain.
...
import { BucketPill, CARD, Filter, SearchBox, TD, TH, TIER_COLOR, TierPill } from './ui';
  ...
  return (
    <div>
      ...
      {mode === 'modules' ? (
        ...
        <RoleRegistry roles={roles} byId={byId} onTrace={onTraceRole} />
      )}
    </div>
  );
}
            ...
            <tbody>
              {shown.map((m) => (
                <tr key={m.id}>
                  ...
                  <td style={{ ...TD, fontWeight: 600, color: 'var(--text-3)', maxWidth: 340 }}>{m.name}</td>
                  <td style={{ ...TD, color: 'var(--text-soft)' }}>{m.type}</td>
                  <td style={TD}>
...
function RoleRegistry({
  ...
  const [q, setQ] = useState('');
  const stats = useMemo(() => new Map(roles.map((r) => [r.id, roleStats(r, byId)])), [roles, byId]);
  const shown = roles
